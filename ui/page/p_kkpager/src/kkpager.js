...
var kkpager = {
		...
		generPageHtml : function(){
			...
			// id: kkpager_btn_go_input
			var gopage_info = '';
			if(this.isGoPage){
				gopage_info = '&nbsp;'+this.lang.gopageBeforeText+'<span id="'+this.gopageWrapId+'">'+
					'<input type="button" id="'+this.gopageButtonId+'"  value="' +this.lang.gopageButtonOkText+'" />'+
					'<input type="text" id="'+this.gopageTextboxId+'" onfocus="kkpager.focus_gopage()" onblur="kkpager.blur_gopage()" value="'+this.next+'" /></span>'+this.lang.gopageAfterText;
			}
			...
			// 拼接完加入dom后，然后绑定事件函数
			$('#'+this.gopageTextboxId).keypress(function(e){
				var kcode = e.which;
				if(kcode==13){
					$('#'+kkpager.gopageButtonId).trigger("click");
				}
				console.log(kcode);				
				if((kcode<48 || kcode>57) && kcode!=8 && kcode !==0){
					return false;
				}
