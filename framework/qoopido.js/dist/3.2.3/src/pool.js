...
(function(definition) {
    ...
    "use strict";
    var prototype, settings = {
        interval: 1e3 / 60,
        frameBudget: .5,
        maxPoolsize: 1e3
    };
    function processQueue() {
        var self = this, metrics = self.metrics, settings = self._settings, queue = self._queue, variables = self._variables, spliceLimit = 1, spliceLength, elements, durationStart;
        if (queue.length > 0) {
            ...
            if ((spliceLength = Math.min(queue.length, (elements = queue.splice(0, spliceLimit)).length)) > 0) {
                if (metrics.inPool + spliceLength <= settings.maxPoolsize) {
                    ...
                    for (var i = 0; i < spliceLength; i++) {
                        var element = elements[i], quid = element._quid, dispose = element.dispose;
