...
angular-ui-router 换成 0.2.15 的版本就不支持ie8了
...
只有使用自定义的【事件回调】或者是第三方类库的【回调】时，才需要自己执行$apply。
    ...
    3、AngularJS进入$digest循环。这个循环是由两个小循环组成的，这两个小循环用来处理$evalAsync队列和$watch列表。
    这个$digest循环直到模型“稳定”前会一直迭代。【这个稳定具体指的是$evalAsync队列为空，并且$watch列表中检测不到任何改变了】。

    4、这个【$evalAsync队列是用来管理那些“----视图渲染前-----需要在当前栈框架外---执行的操作的----”】。

    5、$watch列表是一个表达式的集合，这些表达式可能是自上次迭代后发生了改变的。
    【如果有检测到了有改变，那么$watch函数就会被调用，它通常会把新的值更新到DOM中】。

    6、一旦AngularJS的$digest循环结束，整个执行就会离开AngularJS和Javascript的上下文。
    这些都是在浏览器为数据改变而进行重渲染之后进行的。
    <input ng-model="name">
    <p>Hello {{name}}!</p>
...
视图：它仍然是HTML。浏览器将它解析成DOM， 然后这个DOM会作为输入传递给模板引擎，也就是我们的编译器。
编译器查看其中的指令，找到的指令后，会开始监视指令内容中相应的模型。 这样做，就使得视图能“连续地”更新，不需要模板和数据的重新合并。你的模型也就成了你视图变化的唯一动因。
---------------------------------------------------------------------------
...
字符串替换式：
在编译期间，编译器会用$interpolate服务去检查文本中是否嵌入了表达式。这个表达式会被当成一个监视器一样注册，
并且作为$digest循环中的一部分，它会自动更新。

首先浏览器会用它的标准API将HTML解析成DOM。 你需要认清这一点，因为我们的模板必须是可被解析的HTML。
这是AngularJS和那些“以字符串为基础而非以DOM元素为基础的”模板系统的区别之处。

DOM的编译是有$compile方法来执行的。
这个方法会遍历DOM并找到匹配的指令。一旦找到一个，它就会被加入一个指令列表中，
这个列表是用来记录所有和当前DOM相关的指令的。 一旦所有的指令都被确定了，会按照优先级被排序，并且他们的compile方法会被调用。
指令的$compile()函数能修改DOM结构，并且要负责生成一个link函数（后面会提到）。
$compile方法最后返回一个合并起来的链接函数，这个链接函数是每一个指令的compile函数返回的链接函数的集合。

通过调用链接函数来将模板与作用域链接起来。这会轮流调用每一个指令的链接函数，让每一个指令都能对DOM注册监听事件，
和建立对作用域的的监听。这样最后就形成了作用域和DOM的动态绑定。任何一个作用域的改变都会在DOM上体现出来。
---------------------------------------------------------------------------
...
----------------------
Factory method工厂函数：
工厂函数是用来创建指令的。它只会被调用一次：就是当编译器第一次匹配到相应指令的时候。
你可以在其中进行任何初始化的工作。调用它时使用的是 $injector.invoke所以它遵循所有注入器的规则。

Directive Definition Object 指令定义对象：
指令定义对象给编译器提供了生成指令需要的细节。这个对象的属性有：

-----【编译函数】是用来处理需要【修改模板DOM】的情况的。-----

-----【链接函数】负责【注册DOM事件和更新DOM】。-----
...
指令定义对象给编译器提供了生成指令需要的细节。这个对象的属性有：
    ...
    控制器controller - 控制器的构造对象。这个控制器函数是在预编译阶段被执行的，并且它是共享的，
    其他指令可以通过它的名字得到。这就使得指令间可以互相交流来扩大自己的能力。会传递给这个函数的参数有：
        $scope - 当前元素关联的作用域。
        $element - 当前元素
        $attrs - 当前元素的属性对象。
    ...
    限制restrict -限制指令的声明格式的。如果没有这一项。那就只允许使用属性形式的指令。
